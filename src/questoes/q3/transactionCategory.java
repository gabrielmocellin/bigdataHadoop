...
public class transactionCategory {
    public static void main(String[] args) throws Exception {
        ...
        Path outputFilePath = new Path("output/q3_resultado");

        // Criação do objeto Job com base na configuração criada anteriormente.
        Job hadoopMapReduceJob = Job.getInstance(hadoopConfig, "transactionYear"); // Job j = new Job(hadoopConfig, "teste");

        // Onde são definidas as classes usadas para esse Job
        hadoopMapReduceJob.setJarByClass(transactionCategory.class);
        hadoopMapReduceJob.setMapperClass(transactionCategoryMapper.class);
        hadoopMapReduceJob.setReducerClass(transactionCategoryReducer.class);
